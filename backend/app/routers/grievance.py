...
@router.post("/grievance/", response_model=schemas.GrievanceOut)
def submit_grievance(grievance: schemas.GrievanceCreate, db: Session = Depends(get_db)):
    db_grievance = models.Grievance(**grievance.dict())
    db.add(db_grievance)
    db.commit()
    db.refresh(db_grievance)
    return db_grievance

@router.get("/grievance/{student_id}", response_model=list[schemas.GrievanceOut])
...
@router.put("/grievance/{grievance_id}/status")
